...
class RivalRaceGame extends FlameGame {
  ...
  final List<RacePowerState> playerPowers=[RacePowerState(RacePowerType.fireBurst,charges:2),RacePowerState(RacePowerType.distortionPulse,charges:1),RacePowerState(RacePowerType.slowWindField,charges:1)];
  final List<RacePowerState> rivalPowers=[RacePowerState(RacePowerType.fireBurst,charges:2),RacePowerState(RacePowerType.distortionPulse,charges:1),RacePowerState(RacePowerType.slowWindField,charges:1)];
  ...
  @override void update(double dt){super.update(dt);if(done)return;elapsed+=dt;bannerTimer-=dt;for(final p in [...playerPowers,...rivalPowers]){p.cooldown=math.max(0,p.cooldown-dt).toDouble();}
    final wingScale=1+(wingLevel-1)*.018;final control=1+(wingLevel-1)*.022;player.velocity.x+=input.x*690*control*dt;player.velocity.y+=input.y*520*control*dt;player.velocity.scale(math.max(0,1-dt*4.5).toDouble());player.position+=player.velocity*dt;player.x=player.x.clamp(35,size.x-35).toDouble();player.y=player.y.clamp(size.y*.18,size.y*.86).toDouble();player.angle+=(player.velocity.x/180*.48-player.angle)*math.min(1,dt*7).toDouble();
    final d=director.update(dt:dt,playerProgress:playerProgress,rivalProgress:rivalProgress,powerReady:rivalPowers.any((p)=>p.ready),boss:bossRace);final target=Vector2(size.x*d.targetX,size.y*d.targetY);final delta=target-rival.position;rival.position+=delta*math.min(1,dt*(bossRace?2.7:2.15)).toDouble();rival.angle+=(delta.x/160*.38-rival.angle)*math.min(1,dt*5).toDouble();if(d.boost){rival.boost=.8;rival.react(PilotMood.laughing);}if(d.usePower)useRivalPower();
    ...
    if(player.integrity<=0||rival.integrity<=0){final forcedWin=rival.integrity<=0&&player.integrity>0?true:player.integrity<=0&&rival.integrity>0?false:playerProgress>=rivalProgress;finishRace(forcedWin:forcedWin);}else if(playerProgress>=1||rivalProgress>=1){finishRace();}updateHud();
  }
  ...
  void checkDebris(DebrisComponent d){for(final racer in [player,rival]){if((racer.position-d.position).length<32){d.removeFromParent();racer.integrity=(racer.integrity-(bossRace?17:13)).clamp(0,100).toDouble();racer.react(PilotMood.shocked);if(racer==player){score=math.max(0,score-60);showBanner('DEBRIS HIT');}else{showBanner('RIVAL CLIPPED');}break;}else if((racer.position-d.position).length<55&&d.y>racer.y&&!d.nearMissAwarded){if(racer==player){d.nearMissAwarded=true;score+=45;showBanner('CLOSE CALL +45');player.react(PilotMood.laughing);}}}}
  void collectToken(){tokens++;score+=20;}
  void boostPlayer(){player.boost=1.05;player.react(PilotMood.fistPump);showBanner('TURBO!');}
  void usePower(RacePowerType type){final p=playerPowers.firstWhere((e)=>e.type==type);if(!p.ready)return;p.charges--;p.cooldown=3.2;add(PowerProjectile(position:player.position.clone()..y-=40,type:type,fromPlayer:true));player.react(PilotMood.fistPump);showBanner(switch(type){RacePowerType.fireBurst=>'FIRE BURST',RacePowerType.distortionPulse=>'DISTORTION PULSE',RacePowerType.slowWindField=>'SLOW FIELD'});}
  void useRivalPower(){final p=rivalPowers.firstWhere((e)=>e.ready,orElse:()=>rivalPowers.first);if(!p.ready)return;p.charges--;p.cooldown=4.0;add(PowerProjectile(position:rival.position.clone()..y+=38,type:p.type,fromPlayer:false));}
