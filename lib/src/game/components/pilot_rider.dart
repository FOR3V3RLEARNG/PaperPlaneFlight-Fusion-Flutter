...
class PilotRiderComponent extends PositionComponent {
  ...
  void render(Canvas canvas) {
    super.render(canvas);
    final excited =
        mood == PilotMood.laughing ||
        mood == PilotMood.fistPump ||
        mood == PilotMood.celebrating;
  ...
  void _renderMonkey(Canvas c) {
    c.drawCircle(
      const Offset(9.5, 8.5),
      3.8,
      Paint()..color = character.primary,
    );
    c.drawCircle(
      const Offset(22.5, 8.5),
      3.8,
      Paint()..color = character.primary,
    );
    c.drawCircle(const Offset(16, 9), 7, Paint()..color = character.primary);
    c.drawOval(
      const Rect.fromLTWH(11, 8, 10, 8),
      Paint()..color = character.skin,
    );
    _renderBody(c, x: 9, width: 14);
    c.drawLine(
      const Offset(10, 19),
  ...
  void _renderDuo(Canvas c) {
    ...
    c.drawCircle(
      const Offset(20.5, 8),
      5.5,
      Paint()..color = character.skin.withValues(alpha: .98),
    );
    c.drawArc(
      const Rect.fromLTWH(6, 1.5, 11, 11),
      3.2,
      2.8,
      true,
      Paint()..color = character.hair,
    );
    c.drawArc(
      const Rect.fromLTWH(15, .5, 11, 11),
      3.2,
      2.8,
      true,
      Paint()..color = character.secondary,
    );
    c.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(7, 15, 18, 14),
  ...
  void _renderBody(Canvas c, {required double x, required double width}) {
    c.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(x, 15, width, 14),
        const Radius.circular(5),
      ),
      Paint()..color = character.primary,
    );
    c.drawRect(
      const Rect.fromLTWH(14.5, 16, 3, 12),
      Paint()..color = character.secondary.withValues(alpha: .72),
    );
  }
  ...
  void _renderExpression(Canvas c, Offset center) {
    ...
    final sweep = mood == PilotMood.laughing || mood == PilotMood.celebrating
        ? 2.7
        : 2.0;
    c.drawArc(
      Rect.fromCenter(center: center.translate(0, .8), width: 7, height: 5),
      .25,
      sweep,
      false,
      smile,
