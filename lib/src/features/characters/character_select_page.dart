...
class _CharacterSelectPageState extends State<CharacterSelectPage> {
  ...
  Widget build(BuildContext context) {
    final worldName = raceWorlds
        .firstWhere(
          (w) => w.id == widget.worldId,
          orElse: () => raceWorlds.first,
        )
        .name;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Text(
            ...
            style: Theme.of(
              context,
            ).textTheme.labelLarge?.copyWith(color: FlightColors.cyan),
          ),
          const SizedBox(height: 8),
          _RosterGrid(
            selectedId: playerOne,
            onSelected: (id) => setState(() => playerOne = id),
          ),
          const SizedBox(height: 16),
          if (wingOneIsFromHangar)
            _WingReadout(
              label: widget.bossRace
                  ? 'P1 WING LEVEL (BOSS)'
                  : 'P1 WING LEVEL (HANGAR)',
              value: wingOne,
            ...
            _WingPicker(
              label: 'P1 WING LEVEL',
              value: wingOne,
              onChanged: (v) => setState(() => wingOne = v),
            ),
          if (needsSecond) ...[
            const SizedBox(height: 24),
            Text(
              widget.mode == CharacterSelectMode.rival
                  ? 'AI RIVAL'
                  : 'PILOT TWO',
              style: Theme.of(
                context,
              ).textTheme.labelLarge?.copyWith(color: FlightColors.orange),
            ),
            const SizedBox(height: 8),
            _RosterGrid(
              selectedId: playerTwo,
              onSelected: (id) => setState(() => playerTwo = id),
            ),
            const SizedBox(height: 16),
            if (widget.mode == CharacterSelectMode.rival && widget.bossRace)
              const _WingReadout(
                label: 'RIVAL WING (BOSS)',
                value: 10,
                loading: false,
              )
            else
              _WingPicker(
                label: widget.mode == CharacterSelectMode.rival
                    ? 'RIVAL WING'
                    : 'P2 WING LEVEL',
                value: wingTwo,
                onChanged: (v) => setState(() => wingTwo = v),
              ),
          ],
        ],
      ),
      bottomNavigationBar: SafeArea(
        ...
        child: FilledButton.icon(
          ...
          label: Text(
            widget.mode == CharacterSelectMode.remote
                ? 'Continue to Lobby'
                : 'Start Race',
          ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Upgrade wings in the Hangar',
                style: TextStyle(fontSize: 8, color: Color(0xFFB8C9D8)),
              ),
            ],
          ),
        ),
        ...
        return GridView.builder(
          ...
          itemBuilder: (_, index) {
            ...
            return Semantics(
              ...
              selected: selected,
              label: pilot.availableByDefault
                  ? 'Select ${pilot.name}'
                  : '${pilot.name}, licensed character slot unavailable',
              child: InkWell(
                onTap: pilot.availableByDefault
                    ...
                    : () => showDialog<void>(
                        ...
                        builder: (_) => AlertDialog(
                          ...
                          content: Text(
                            pilot.licenseNote ??
                                'This character requires licensing.',
                          ),
                          actions: [
                            TextButton(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: selected
                        ? pilot.primary.withValues(alpha: .13)
                        : FlightColors.panel,
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(
                      color: selected ? pilot.primary : Colors.white12,
                      width: selected ? 2 : 1,
                    ),
                  ),
                  child: Column(
                    ...
                    children: [
                      Expanded(
                        child: Center(child: _PilotPortrait(pilot: pilot)),
                      ),
                      Text(
                        pilot.name,
                        maxLines: 1,
...
class _PilotPortrait extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      CustomPaint(size: const Size(74, 74), painter: _PortraitPainter(pilot));
}
...
class _PortraitPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      c,
      size.width * .46,
      Paint()..color = pilot.primary.withValues(alpha: .11),
    );
    switch (pilot.species) {
      case PilotSpecies.human:
        canvas.drawCircle(
          Offset(c.dx, c.dy - 7),
          17,
          Paint()..color = pilot.skin,
        );
        canvas.drawArc(
          Rect.fromCircle(center: Offset(c.dx, c.dy - 12), radius: 18),
          3.3,
          2.8,
          true,
          Paint()..color = pilot.hair,
        );
        break;
      case PilotSpecies.pinkCat:
        final face = Paint()..color = pilot.skin;
        final ears = Path()
          ..moveTo(c.dx - 17, c.dy - 14)
          ..lineTo(c.dx - 10, c.dy - 32)
          ..lineTo(c.dx - 1, c.dy - 15)
          ..moveTo(c.dx + 3, c.dy - 15)
          ..lineTo(c.dx + 12, c.dy - 32)
          ..lineTo(c.dx + 18, c.dy - 13);
        canvas.drawPath(
          ears,
          Paint()
            ..color = pilot.primary
      ...
      case PilotSpecies.monkey:
        canvas.drawCircle(
          Offset(c.dx - 16, c.dy - 7),
          8,
          Paint()..color = pilot.primary,
        );
        canvas.drawCircle(
          Offset(c.dx + 16, c.dy - 7),
          8,
        ...
        canvas.drawOval(
          Rect.fromCenter(center: Offset(c.dx, c.dy), width: 24, height: 18),
          Paint()..color = pilot.skin,
        );
        break;
      case PilotSpecies.duo:
        canvas.drawCircle(
          Offset(c.dx - 11, c.dy - 9),
          14,
          Paint()..color = pilot.skin,
        );
        canvas.drawCircle(
          Offset(c.dx + 11, c.dy - 6),
          14,
          Paint()..color = pilot.skin.withValues(alpha: .96),
        );
        break;
    }
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromCenter(center: Offset(c.dx, c.dy + 25), width: 42, height: 24),
        const Radius.circular(10),
      ),
      Paint()..color = pilot.secondary,
    );
    canvas.drawCircle(
      Offset(c.dx - 6, c.dy - 7),
      1.5,
