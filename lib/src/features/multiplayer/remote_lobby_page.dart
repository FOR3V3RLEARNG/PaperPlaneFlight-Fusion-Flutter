...
class _RemoteLobbyPageState extends State<RemoteLobbyPage> {
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: pilot.primary.withValues(alpha: .35)),
            ),
            child: Row(
              children: [
                CircleAvatar(
                  backgroundColor: pilot.primary.withValues(alpha: .2),
                  child: Icon(Icons.face_rounded, color: pilot.primary),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Wing ${widget.selection?.wingLevel ?? 1} • ${pilot.callSign}',
          ...
          FilledButton.icon(
            onPressed: () => _notConnected(context),
          ...
          const SizedBox(height: 18),
          const Text(
            'The gameplay protocol is separated from transport. A Supabase Realtime adapter can synchronize racer position, powers, debris seed and finish state without changing the Flame race rules.',
            style: TextStyle(
              fontSize: 10.5,
              color: Color(0xFFB8CBDC),
              height: 1.45,
  ...
  void _notConnected(
    ...
    const SnackBar(
      content: Text(
        'Remote transport is ready for Supabase Realtime wiring; no credentials are embedded in this build.',
      ),
    ),
  );
}
